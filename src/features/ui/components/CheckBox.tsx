  ...
  return (
    <label className={'inline-flex items-center group cursor-pointer ' + props.className}>
      <div className="relative flex items-center rounded-full transition group-active:translate-y-0.5">
        <input type="checkbox" {...props} className={[inputBaseClasses, sizeClasses[size], `checked:bg-${color} checked:border-${color}`].join(' ')} />
        ...
        <div
          ...
          className={`bg-gray bg-opacity-0 group-hover:bg-opacity-10 peer-checked:bg-opacity-0 peer-checked:hover:bg-opacity-10 peer-checked:bg-${color} w-12 h-12 rounded-full absolute top-2/4 left-2/4 -translate-y-2/4 -translate-x-2/4 transition-opacity`}
        ></div>
      </div>
